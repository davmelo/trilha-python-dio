...
class Cliente:
    ...
    def realizar_transacao(self, conta, transacao):
        if len(conta.extrato.transacoes_do_dia()) >= 10:
            console.print(
                "[red]Operação não realizada! Limite de operações diárias atingido.[/red]"
            )
...
class Conta:
    ...
    def sacar(self, valor):
        ...
        if excedeu_saldo:
            console.print(
                "[red]Operação não realizada! Você não tem saldo suficiente.[/red]"
            )
            return False
            ...
            console.print(
                "[red]Operação não realizada! O valor informado é inválido.[/red]"
            )
            return False
...
class ContaCorrente(Conta):
    ...
    def sacar(self, valor):
        ...
        if excedeu_limite:
            console.print(
                "[red]Operação não realizada! O valor do saque excede o limite por operação.[/red]"
            )
            return False

        elif excedeu_saques:
            console.print(
                "[red]Operação não realizada! Limite de saques diários atingido.[/red]"
            )
            return False
...
def mensagem_nao_possui_conta(nome_completo):
    ...
    print()
    console.print(
        f"[yellow]Sr(a) {nome}, você ainda não possui uma conta em nosso banco. Crie uma antes de realizar alguma movimentação.[/yellow]"
    )
...
def mensagem_nao_cliente():
    print()
    console.print(
        "[yellow]Você ainda não é cliente do Banco Python. Cadastre-se antes de criar uma conta.[/yellow]"
    )
...
def mensagem_sem_transacoes(conta):
    print()
    console.print(
        "[bold cyan]╠═════════════════════   EXTRATO   ═════════════════════╣[/bold cyan]"
    )
    print()
    console.print(
        f" [green]Titular: {conta.cliente.nome}  -  Agência: {conta.agencia}  C/C: {conta.numero}[/green]"
    )
    print()
...
def depositar(clientes):
    if not (cliente_e_contas := verificacao_padrao(clientes)):
        ...
        numero_conta = int(
            Prompt.ask(
                "[green]Insira o número da conta onde será realizado o depósito[/green]"
            )
        )
...
def sacar(clientes):
    if not (cliente_e_contas := verificacao_padrao(clientes)):
        ...
        numero_conta = int(
            Prompt.ask(
                "[green]Insira o número da conta onde será realizado o saque[/green]"
            )
        )
        conta = procurar_conta(cliente_e_contas["cliente"], numero_conta)
...
def percorrer_contas_extrato(contas):
    ...
    for conta in contas:
        (
            mensagem_sem_transacoes(conta)
            if not conta.extrato.transacoes
            else exibir_extrato(conta, opcao)
        )
...
def extrato_uma_ou_todas(cliente):
    try:
        ...
        match opcao:
            case 1:
                numero_conta = int(
                    Prompt.ask("[green]Insira o número da conta[/green]")
                )
                ...
                if conta:
                    (
                        mensagem_sem_transacoes(conta)
                        if not conta.extrato.transacoes
                        else exibir_extrato(conta, tipo_de_transacao_extrato())
                    )
...
def ler_dados_cliente():
    ...
    data_nascimento = str(
        Prompt.ask("[green]Digite sua data de nascimento (dd/mm/aa)[/green]")
    )
    endereco = str(
        Prompt.ask(
            "[green]Digite seu endereço (logradouro, número, distrito, cidade/SIGLA ESTADO)[/green]"
        )
    )
...
def criar_conta(clientes):
    ...
    if not cliente_encontrado:
        ...
        conta_corrente = ContaCorrente.nova_conta(
            cliente=cliente_encontrado, numero=numero_de_contas
        )
